...
//返回B1 xor B2结果
int bit_xor(char *array, int index) {
  index = index / 2;
...
//bit代表B1 xor B2结果，改变其中一个则改变xor结果
...
bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    bit_convert(bd_sizes[k].alloc, bi);  // free p at size k
    if (bit_xor(bd_sizes[k].alloc, buddy)) {  // is buddy allocated?
...
// //free则返回1，否则返回0
int
bd_isfree(int k, int bi, void *left, void *right){
  if((addr(k,bi) >= left) && (addr(k,bi) + BLK_SIZE(k) < right))
    return 1;
  else
    return 0;
}
...
bd_initfree_pair(int k, int bi, void *bd_left, void *bd_right) {
  int buddy = (bi % 2 == 0) ? bi+1 : bi-1;
  int free = 0;
  if(bit_xor(bd_sizes[k].alloc, bi) == 1) {
    // one of the pair is free
    free = BLK_SIZE(k);
    if(bd_isfree(k,bi,bd_left,bd_right))
